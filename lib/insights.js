...
export function buildDayDetail(day, habits) {
  ...
  return {
    ...
    tone,
    headline,
    groupedEntries: Array.from(groupedEntries.values()).sort((a, b) => {
      if (!a.time) return 1;
      if (!b.time) return -1;
...
export function buildRiskForecast(historyDays = []) {
  ...
  const hasEveningData = totalOldEntries >= 5 && eveningOldEntries > 0;
  const eveningShare = hasEveningData ? Math.round((eveningOldEntries / totalOldEntries) * 100) : 0;

  const riskScore = oldRun * 2 + quietRun + Math.max(recentOld - recentNew, 0) + (hasEveningData && eveningShare >= 50 ? 1 : 0);
  ...
  const thirdChip = hasEveningData
    ? { label: "Abenddruck", value: `${eveningShare}%`, tone: eveningShare >= 50 ? "red" : eveningShare >= 25 ? "neutral" : "gold" }
    : { label: "Rückfall-Lauf", value: `${oldRun} Tage`, tone: oldRun >= 3 ? "red" : oldRun >= 1 ? "neutral" : "gold" };

  const cap = (t) => tone === "gold" && t === "red" ? "neutral" : t;
  ...
  return {
    ...
    lead,
    chips: [
      { label: "5-Tage-Saldo", value: formatSigned(recentNet), tone: cap(recentNet > 0 ? "gold" : recentNet < 0 ? "red" : "neutral") },
      { label: "Ohne Aufbau", value: `${quietRun} Tage`, tone: cap(quietRun >= 2 ? "red" : quietRun === 1 ? "neutral" : "gold") },
      { ...thirdChip, tone: cap(thirdChip.tone) },
    ],
    signals: [
      weekdayRisk?.old > 0 ? `Risikotag: ${weekdayRisk.weekday}.` : "Kein Wochentag auffällig.",
      oldRun > 0 ? `${oldRun} Tage am Stück mit altem Muster.` : "Kein aktiver Rückfall-Lauf.",
      quietRun > 1 ? `${quietRun} Tage ohne Aufbau.` : "Aufbau läuft durch.",
    ],
  };
}
...
export function buildMonthlyRadar(historyDays = []) {
  ...
  const summaries = months.map(([key, days]) => {
    ...
    return {
      ...
      raw: {
        aufbau: totalNew,
        saldo: Math.max(totalNew - totalOld, 0),
        konstanz: activeDays,
  ...
  const axes = [
    ...
    { key: "saldo", label: "Saldo", hint: "Netto-Aufbau: Aufbau-Einträge abzüglich Rückfälle, nur positiver Anteil." },
    { key: "konstanz", label: "Konstanz", hint: "An wie vielen Tagen du etwas festgehalten hast." },
    { key: "fokus", label: "Fokus", hint: "Tage mit mehr Aufbau als Rückfall." },
  ];
    ...
    cards: [
      {
        ...
        meta: `${habits.new.length} Fäden aktiv`,
        tone: "gold",
      },
      {
        label: "Rückfall",
        value: totalOldEntries,
        meta: `${habits.old.length} Fäden beobachtet`,
        tone: "red",
      },
      {
        label: "Streak",
        value: stats.streak || 0,
        meta: (stats.streak || 0) > 0 ? "Tage in Folge" : "noch kein Lauf",
        tone: (stats.streak || 0) >= 3 ? "gold" : "neutral",
      },
      {
        label: "Anruf-Serie",
        value: callStreak,
        meta: callStreak > 0 ? "Werktage am Stück" : "kein aktiver Lauf",
