...
function _calcStreak(db, starId, todayStr) {
  const starHabits = selectAll(db, "SELECT habit_id FROM star_habits WHERE star_id = ? AND active = 1", [starId]);
  if (!starHabits.length) return 0;

  const ids = starHabits.map((sh) => sh.habit_id);
  ...
  for (let offset = 0; offset < 365; offset++) {
    ...
    const row = selectOne(db, `
      SELECT COUNT(*) AS count FROM (
        SELECT 1 FROM entries WHERE entry_date = ? AND habit_id IN (${ph})
        GROUP BY entry_date HAVING COUNT(DISTINCT habit_id) = ?
      )
    `, [dateStr, ...ids, n]);
    const complete = (row?.count || 0) > 0;
    if (complete) {
      streak++;
    } else if (offset === 0) {
      // heute noch nicht vollständig — trotzdem gestern prüfen
    } else {
      break;
    }
  }
...
function _calcCompletedDays(db, starId, unlockedDateStr, todayStr) {
  const starHabits = selectAll(db,
    "SELECT habit_id FROM star_habits WHERE star_id = ? AND active = 1",
    [starId]
  );
  if (!starHabits.length) return 0;
  ...
  `, [unlockedDateStr, todayStr, ...ids, ids.length]);
  return row?.count || 0;
}
...
function _checkUnlocks(db) {
  ...
  for (let i = 1; i < stars.length; i++) {
    ...
    const completedDays = _calcCompletedDays(db, prev.id, unlockedDateStr, todayStr);
...
export async function getStarsStatus() {
  ...
  return stars.map((star) => {
    ...
    const completedDays = _calcCompletedDays(db, star.id, unlockedDateStr, todayStr);
